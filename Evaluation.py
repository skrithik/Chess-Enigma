...
class Evalualtion():
    ...
    def __init__(self, board, color):
        self.board = board
        self.color = color
        self.boardValues = BoardValues()## Loads predefined piece development values.
        self.lateGameWhite = False
        self.lateGameBlack = False

        #Cretes empty BoardLayout list from where comparisons can be made.
        self.boardLayout = [None] * 64## Creates an empty board representation (64 squares).

        # Actually populate boardLayout with data from boardString.
        self.populate() # Fills `boardLayout` with the current board state.
    ...
    def development(self):#Method (Evaluates Piece Placement & Game Stage)
        total = 0
        counter = 0
